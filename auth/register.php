...
#loged in? cheking using username
if(isset($_SESSION['username'])){
  echo "<script>window.location.href='".APPURL."' </script>";
}  
  #else taking the value
  if(isset($_POST['submit'])){
    if(empty($_POST['username']) OR empty($_POST['email']) OR empty($_POST['password'])){
      ...
      #prepare? Prepared statements are precompiled SQL 
      ...
      #then finally calling executing on the prepare statement
      $insert->execute([
        ":username"=> $username,
        ":email"=> $email,
        ":mypassword"=> $password,
      ...
      #after completing redecting the user to the login.php link
