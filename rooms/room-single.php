	...
	if(isset($_GET['id'])) {
		...
		if(isset($_POST['submit'])){
			...
			OR empty($_POST['check_in']) OR empty($_POST['check_out'])){
				...
				$check_in_str = date_format($check_in, 'Y-m-d'); //this line and next line added later to handle date issues
				$check_out_str = date_format($check_out, 'Y-m-d');
			
				if($check_in_str < date('Y-m-d') || $check_out_str < date('Y-m-d')) {
					echo "<script>alert('Ooops! Pick a Valid Date, that is not from the past!')</script>";
				} else{
				
					if($check_in_str <= date('Y-m-d')) {
						echo "<script>alert('Noo! You can only check-in starting from tomorrow.')</script>";
	    		...
	    		<div class="col-lg-4">
					<form action="room-single.php?id=<?php echo $id; ?>" method="POST" class="appointment-form" style="margin-top: -568px;">
						...
						<div class="row">
							...
							<?php else : ?>
								<p>Login or Register in order to book a room</p>
							<?php endif; ?>
						</div>
				</form>
	    		</div>
